...
public class GameService {
	...
	public GameService(@Autowired GameStore store) {
		this.store = store;
	}
	...
	public void resetMatch() {
		...
		resetPlayerCards(team1.getPlayer1());
		resetPlayerCards(team1.getPlayer2());
		resetPlayerCards(team2.getPlayer1());
		resetPlayerCards(team2.getPlayer2());
	}
	...
	String calculateWinner(Round round) {
		...
		for (Entry<String, Card> entry : round.getMoves().entrySet()) {
			...
			} else if (currentCard.getType() == round.getRanga().getType() && !round.getRanga().equals(Card.JOKER)) {
				winner = entry;
			}
